...
foto = open("C:/Users/devnu/Downloads/Hikxploit-master/oto_programmate.txt", "rb").read().splitlines()
testi_orari = open("C:/Users/devnu/Downloads/Hikxploit-master/esto_orari.txt", "rb").read().splitlines()
	...
	if content_type == 'text' and acc:
		...
		if text[0] == '/new':
			try:
				testo_pic = text[1]
				orario_pic = text[2]
				with open("C:/Users/devnu/Downloads/Hikxploit-master/esto_orari.txt", "wb") as testOrari:
			...
			foto_aggiornato_lista = open("C:/Users/devnu/Downloads/Hikxploit-master/oto_programmate.txt",'rb').read().splitlines()
			testi_aggiornato = open("C:/Users/devnu/Downloads/Hikxploit-master/esto_orari.txt", 'rb').read().splitlines()
			print(foto_aggiornato_lista)
			print(testi_aggiornato)
			try:
				for i in range(len(foto_aggiornato_lista)):
		...
		elif text[0] == '/reboot' and int(user_id) == int(130752603): # perché converti un intero in un intero di nuovo?
			bot.sendMessage(chat_id, "Reboot in corso...")
			time.sleep(2)
		elif text[0] == '/edit_testo':
			bot.sendMessage(chat_id,"HO DETTO WIP PORCA PUTTANA!")
		elif text[0] == '/edit_orari':
		...
		print(foto)
		with open("C:/Users/devnu/Downloads/Hikxploit-master/oto_programmate.txt", "wb") as foto_download:
			foto_download.write(username + random + '.png')
			foto_download.write("\n")
		foto_aggiornato = open("C:/Users/devnu/Downloads/Hikxploit-master/oto_programmate.txt", "rb").read().splitlines()
		...
		bot.sendMessage(chat_id, (
			"ora usa il comando /new (testo) + (orario) per impostare a che ora pubblicare ciò, ricorda il tempismo è fondamentale per non confodersi\n" +
			"quindi hai esattamente 180 secondi per inserire un testo anche base,dopo di che la pic verrà eliminata"
		))
	elif content_type == 'text' and not acc:
